...
Last grounded: 2026-06-19. Re-verify before trusting; model leaderboards move weekly.
...
| `low` | light | orchestrator `default` on Opus/Fable — judgment is cheap for these models; fan-out carries the load, keeps orchestrator latency/context low |
| `medium` | moderate | GPT-5.5 `default`; mid-weight `task`/`vision` |
...
|---|---|---|---|---|---|---|
| `default` | O4.8:low | O4.8:low | O4.8:low | G5.5:medium | G5.5:medium | F5:low |
...
|---|---|---|---|
| `config.yml` (active) | Opus 4.8 `:low` | **9.0** | Production default. Opus orchestrates, escalates `plan`/`slow`/`task` to GPT-5.5. |
| `claude.yml` | Opus 4.8 `:low` | **9.0** | All-Anthropic; protects OpenAI quota. `task` drops to Sonnet 4.6. |
| `combo-claude.yml` | Opus 4.8 `:low` | **9.0** | Claude owns context; GPT-5.5 reserved for burst reasoning / design. |
